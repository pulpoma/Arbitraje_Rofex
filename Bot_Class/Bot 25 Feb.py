...
class StrategyArbitrage:
            ...
            for offer in dataframe_tasa_ws['offer']:
                ...
                if bid > offer and (self.fire_power > (self.size * 2)):
                    ...
                    if key1 in self.keys:
                        ...
                        """Calculo si tengo suficiente saldo para realizar las operaciones"""
                        if saldo_left > ( (self.size * price_offer) + (self.size * price_bid) ):
    ...
    def market_data_handler(self, message):
        ...
        Le agrego 0.01 porque a veces las operaciones se traban un poco y llegan tarde>para tener margen en la operacion
         (en un caso real se sacaria)"""
        bidc = (bidx - .01) * (1 - self.commission)
        offerc = (offerx  + .01) * (1 + self.commission)
        self.future_ws_df_comission.loc[symbol] = [bidc,offerc,last,monthx,yearx,datex]
