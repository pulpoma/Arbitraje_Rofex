            ...
            for bid in dataframe_tasa_ws['bid']:
                ...
                if bid > offer and (self.fire_power > (self.size * 2)):
                    ...
                    if self.count == 0:
                        ...
                        """Busco en el df de ordenes activas si hay algun activo igual a los que llegan en mensajes
                        nuevos y se estan iterando. Esto es solo despues de haber mandado por lo menos una orden antes.
                        Para estos activos, busco el ID interno y luego busco los que tienen igual ID. Asi puedo
                        encontra la combinacion para el mismo arbitraje (bid-offer) y no repetirlo, si es que sigue
                ...
                for bid in df['bid']:
                    ...
                    if bid >= offer and (ticker_bid_new != ticker_offer_new):
                        intern_id_bid_list = self.order_active[self.order_active['ticker'] == ticker_bid_new][
                            'intern_id']  #de las ordenes activas donde esta el ticker del bid que se esta iterando
                        intern_id_offer_list = self.order_active[self.order_active['ticker'] == ticker_offer_new][
                            'intern_id']  #de las ordenes activas donde esta el ticker del offer que se esta iterando
                        ...
                        for id_bid in intern_id_bid_list:
                            for id_offer in intern_id_offer_list:
                                ...
                                if id_bid == id_offer:
                                    ...
                                    """"arb_to_close_intern_id_list" es una lista porque dos operaciones van a tener el
                                    mismo id dado que id_bid==id_offer. Por eso despues tengo que iterar para que se 
                                    cierren las dos operaciones. b_o simplemente es la iteracion para cerrar las
                                    operaciones de venta en bid y compra en offer"""
                                    ...
                                    print(f"Tested Key: {key1}")
                                    print(f"Existing Key to be closed: {key2}")
                                    ...
                                    if key1 == key2:
                                        ...
                                        for b_o in range(0, 2):
                                            order_id = arb_to_close_intern_id_list.iloc[
                                                b_o].name  # id de operacion compra/venta original
                                            ...
                                            price_0 = self.future_ws_df[self.tasas['offer'] == offer]['offer'][0]
                                            price_1 = self.future_ws_df[self.tasas['bid'] == bid]['bid'][0]
